...
x variable
        ...
        """
        if type(value) is not int:
                        raise TypeError("x must
be an integer")
        elif value < 0:
                        raise ValueError("x must
be >= 0")
        else:
                        self.__x = value
...
self.width,
    ...
    def update(self, *args, **kwargs):
                """Method that changed the order
of arguments for rectangle object
        Args:
        ...
        Return:
                       Always nothing
        """
        dict_order = ['id', 'width', 'height',
'x', 'y']
        if args is not None and bool(args) is
True:
                i = 0
