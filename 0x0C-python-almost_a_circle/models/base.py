...
class Base:
    ...
    def __init__(self, id=None):
                """Method that assign the public
instance attribute id
        Args:
                       id(int): integer value to
manage id in this project
        Return:
                       Always nothing.
        """
        if id is not None:
...
bool(list_dictionaries) is False:
                return "[]"
        else:
                        json_string =
...
bool(json_string) is False:
                json_string = "[]"
        return json.loads(json_string)

    @classmethod
    def create(cls, **dictionary):
                """Update the class Base and
returns a instance with all
           attributes already set
        Args:
                       dictionary: Dictionary
with all attributes of the object
        Return:
                       A instance with all
attributes already set
        """
...
instances
           - the type of these instances depends
             on cls
        """
        filename = "{}.json".format(cls.__name__)
        instance_list = []
...
== 'Rectangle':
                        result =
csv.DictWriter(f, fieldnames=rectangle_header)
                elif cls.__name__ == 'Square':
                                        result =
csv.DictWriter(f, fieldnames=square_header)
                result.writeheader()
                result.writerows(data)

    @classmethod
    def load_from_file_csv(cls):
                """Method that deserializes in
CSV
        """
        filename = "{}.csv".format(cls.__name__)
        instance_list = []
...
of square objects
           list_rectangles(list): List of
rectangle objects
        Return:
                       Always nothing
        """
        # Open screen and set the turtle in the
        # center
        s = turtle.getscreen()
        t = turtle.Turtle()

        # Add a title to my screen
        turtle.title("My first draw with python
and tutle module")
        ...
        t.pen(pencolor="blue", fillcolor="white",
                pensize=5, speed=1)
        # Extract the data from the instance
        # rectangle list
        for instance in list_rectangles:
                        # Customize pen for
                        # rectangle
            t.pen(pencolor="blue",
                    fillcolor="white", pensize=5,
                    speed=1)
            data = instance.to_dictionary()
            # Set the position acording the
            # rectangle object
            t.home()
        ...
        t.pen(pencolor="red", fillcolor="white",
                pensize=5, speed=0.5)
        # Extract the data from the instance
        # square list
